...
function createPlayer($container){
    ...
    $player.src = "images/baguette.png";
...
function createEnemy($container, x, y){
    ...
    $enemy.src = "images/enemy3.png";
    $enemy.className = "enemy";
    $container.appendChild($enemy);
    const enemy_cooldown = Math.floor(Math.random() * 50);
...
function updateEnemies(){
    const dx = Math.sin(Date.now() / 1000) * 60;
    const dy = Math.cos(Date.now() / 1000) * 50;
    const enemies = STATE.enemies;
...
function createEnemies($container){
    ...
    for (let i = 0; i < STATE.number_of_enemies/2; i++){
        createEnemy($container, i * 90, 220);
    }
}
